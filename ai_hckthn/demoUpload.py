...
while True:
    ...
    if (audio_proc.poll() != None) and (now - image_time >= AUDIO_INTERVAL):
        ...
        audio_time = now
        audio_gmtime = time.gmtime()
        audio_proc = subprocess.Popen(['echo {} {}'.format(AUDIO_INTERVAL, audio_name)], shell=True)
    ...
    if (image_proc.poll() != None) and (now - image_time >= IMAGE_INTERVAL):
        ...
        image_time = now
        image_gmtime = time.gmtime()
        image_proc = subprocess.Popen(["echo {}".format(image_name)],shell=True)
